...
{
    //pierwszą wersję zrobił Rafał Linowiecki w ramach pracy mgr
    //poprawki (szczególnie kalibracji) zrobił Dawid Gruszczyński

    using GameLab.Geometry;
    using OpenEyeGazeInterface;
    ...
    public class OpenEyeGazeEyetracker : ICalibratableEyetracker
    {
        ...
        private bool pauseAfterCalibrationFinished;

        private static ManualResetEvent manualResetEvent = new ManualResetEvent(true); //wstrzymywanie wątku odbierającego dane
        ...
        public OpenEyeGazeEyetracker(int screenWidth, int screenHeight, bool leftEyeEnabled = true, bool rightEyeEnabled = true, bool bestEyeEnabled = true, int refreshTime = 10, int averagingTime = 100, bool pauseAfterCalibrationFinished = true) //przedostatnie to okres uśredniania
        {
            ...
            this.pauseAfterCalibrationFinished = pauseAfterCalibrationFinished;
        ...
        */
        ...
        public bool Calibrate(int numberOfCalibrationPoints, int displayDeviceIndex, string imageFilePath, int imageSize, bool increasedSpeed, bool acceptationRequiredForEveryCalibrationPoints, int backgroundBrightness, ref string message, ICustomCalibrationScreen customCalibrationScreen)
        {
            ...
            Calibration calibration = new Calibration();
            int pauseAfterCalibrationLength = 10000; //10s
            //calibration.CALIBRATE_FAST(increasedSpeed); //usunięte z GazePoint API 2.0
            try
            {
                if (customCalibrationScreen != null)
                {

                    calibration.CALIBRATE_TIMEOUT(4);

                    customCalibrationScreen.Settings = EyetrackerCalibrationSettings.Default;
                    //new Thread(() => { customCalibrationScreen.ShowDialog(); }).Start();
                    customCalibrationScreen.Show();
                    // customCalibrationScreen.ShowDialog();

                    calibration.CALIBRATE_START(true);

                    for (int i = 0; i < numberOfCalibrationPoints; i++)
                    {
                        Calibration.CALIB_PT_STRUCT pt_struct;
                        calibration.CALIB_START_PT(out pt_struct);
                        int x = (int)(pt_struct.CALX * 100 * customCalibrationScreen.ScreenSize.Width / 100);
                        int y = (int)(pt_struct.CALY * 100 * customCalibrationScreen.ScreenSize.Height / 100);

                        customCalibrationScreen.CalibrationPoint = new CalibrationPoint() { Position = new Point(x, y) };
                    }
                    calibration.CALIB_RESULT(out calibrationResult);
                    calibration.CALIB_RESULT_SUMMARY(out summaryCalibrationResult);
                    calibration.CALIBRATE_START(false);
                    if (pauseAfterCalibrationFinished) Thread.Sleep(pauseAfterCalibrationLength);
                    customCalibrationScreen.Close();
                }
                else
                {
                    calibration.CALIBRATE_SHOW(true);
                    calibration.CALIBRATE_START(true);
                    calibration.CALIB_RESULT(out calibrationResult);
                    calibration.CALIB_RESULT_SUMMARY(out summaryCalibrationResult);
                    if(pauseAfterCalibrationFinished) Thread.Sleep(pauseAfterCalibrationLength);
                    calibration.CALIBRATE_START(false);
                    calibration.CALIBRATE_SHOW(false);
                }
            }
            catch (Exception ex)
            {
                message = $"Problem z urządzeniem: {ex.Message}";
            }
            OnCalibrationOrValidationFinished(true);

            manualResetEvent.Set(); // wznowienie wątku odbierającego dane na temat położenia oka
            return true;
        }
